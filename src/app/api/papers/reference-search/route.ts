    ...
    const readable = new ReadableStream({
      async start(controller) {
        ...
        try {
          ...
          if (titles.length === 0) {
            send({ type: "error", error: "未能从参考文献中提取到论文标题" });
          ...
          const enriched = foundPapers.length > 0
            ? await enrichPapersBatch(foundPapers)
            : [];
