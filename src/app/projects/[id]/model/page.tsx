...
export default function ConceptualModelPage() {
  ...
  const { savedData: savedModel, savedAt: modelSavedAt, save: saveModel, loaded: modelLoaded } = useSavedAnalysis<{ nodes: Node[]; edges: Edge[] }>(projectId, "model");
  ...
  useEffect(() => {
    const isInitial = nodes.length === INITIAL_NODES.length && nodes.every((n, i) => n.id === INITIAL_NODES[i]?.id);
    if (modelLoaded && savedModel && isInitial) {
      if (savedModel.nodes) setNodes(savedModel.nodes);
      if (savedModel.edges) setEdges(savedModel.edges);
    }
  }, [modelLoaded, savedModel]);
      ...
      <div className="flex items-center justify-between">
        ...
        {modelSavedAt && (
          <span className="text-[10px] text-muted-foreground">
            已保存 {new Date(modelSavedAt).toLocaleString("zh-CN")}
          </span>
        )}
